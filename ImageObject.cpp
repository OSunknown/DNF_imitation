...
void ImageObject::Init(string name, D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scale)
{
	InitializeBuffers();
...
void ImageObject::Init(D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scale)
{
	InitializeBuffers();
	_position = position;
	_rotation = rotation;
...
void ImageObject::Init(D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scale, Animator controller)
{
	InitializeBuffers();
	_position = position;
	_rotation = rotation;
...
void ImageObject::Shutdown()
{
	_vertexBuffer->Release();
	_indexBuffer->Release();
}

void ImageObject::SetPosition(D3DXVECTOR3 position)
{
	isTransfromChange = true;
	_position = position;
}

void ImageObject::SetRotation(D3DXVECTOR3 rotation)
{
	isTransfromChange = true;
	_rotation = rotation;
}

void ImageObject::SetScale(D3DXVECTOR3 scale)
{
	isTransfromChange = true;
	_scale = scale;
}

void ImageObject::AddOriginalPosition(D3DXVECTOR3 position)
{
	isTransfromChange = true;
	_position += position;
}

int ImageObject::GetIndexCount()
{
	return m_indexCount;
}

D3DXVECTOR3 ImageObject::GetPosition()
{
	return _position;
}

D3DXMATRIX ImageObject::GetWorldMatrix()
{
...
ID3D11ShaderResourceView* ImageObject::GetTexture()
{
...
void ImageObject::RenderBuffers()
{
...
void ImageObject::SetRect()
{
	...
	//(float)((m_screenWidth / 2) * -1) + (float)positionX;
	if (Filp)
	{
		// Screen¿¡¼­ 
